...
from scine_autocas.interfaces.chronusq.cq_hdf5_reader import CQ_HDF5_Reader
...
class ChronusQ(Interface):
    ...
    class Settings(Interface.Settings):
        ...
        def __init__( self, molecule: Molecule, settings_dict: Optional[Dict]=None ):
            ...
            """
            int
                Variable that determines the number of GMRES iterations in CASPT2 (this helps in determining the amount of memory)
            """
    ...
    def __init__(self, molecule: Molecule, settings_dict: Optional[Dict[str, Any]] = None):
        ...
        """
        int
            Variable that stores the number of jobs that have been proformed so far in order to place the calculations in different directories
        """
    ...
    def caspt2_frozen_orbitals(self, nAO: int, nAE: int):
        ...
        for i in range(nCore):
            ...
            if mem < memory_limit:
                ...
                return i, i*ratio
        raise RuntimeError("Could not find an optimal number of frozen orbitals for CASPT2 (perhaps you can request more memory)")
    ...
    def run_chronusq(self):
        ...
        if os.system(exe_cmd) != 0:
            raise RuntimeError("Chronus Quantum Job failed. See " + self.scratch_dir + " for details.\n Command: %s" %(exe_cmd) )
        ...
        if len(self.settings.broombridge_file) > 0:
           ...
           shutil.copyfile(self.settings.broombridge_file, self.settings.work_dir + "/" + self.settings.broombridge_file)
    ...
    def calculate(self, cas_occupation: Optional[List[int]] = None, cas_indices: Optional[List[int]] = None
        ...
        # Read QCMaquis data
        if (self.settings.method == "dmrg_ci" or self.settings.method == "dmrg_scf") and not self.settings.broombridge_only and not is_hf:
