...
try:
    import qcserenity.serenipy as spy  # type: ignore
...
class Serenity(Interface):
    ...
    def __init__(self, molecules: List[Molecule], settings_dict: Optional[Dict[str, Any]] = None, load_paths=[]):
        ...
        if not load_paths:
            super().__init__(molecule=molecules[0], settings_dict=settings_dict)
            ...
            for name, xyz_file in zip(self.settings.system_names, xyz_files):
                ...
                assert molecules[0].spin_multiplicity > 0
            ...
            for path, name in zip(load_paths, settings_dict["Interface"]["system_names"]):
                settings = spy.Settings()
                settings.load = path
                settings.name = name
            ...
            settings_dict["Interface"]["xyz_files"] = xyz_files
            self.molecules = loaded_molecules
            super().__init__(molecule=loaded_molecules[0], settings_dict=settings_dict)
    ...
    def build_fragments(self, sys):
        """Build fragments for the system. The number of fragments is
        determined by the number of partitioning thresholds plus one.
