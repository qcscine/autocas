...
class Diagnostics:
    ...
    def __init__(self) -> None:  # , settings_dict: Optional[Dict[str, Any]] = None):
        ...
        """
        self.single_reference_threshold: float = Defaults.AutoCAS.single_reference_threshold
        """if max(s1) is lower then this threshold the system will be defined as single reference"""
        self.weak_correlation_threshold: float = Defaults.AutoCAS.weak_correlation_threshold
    ...
    def is_single_reference(self, s1_entropy: np.ndarray) -> bool:
        """Check if the system is weak correlated.

        Meaning the maximum s1 value is lower than 10% of the theoretical maximum, e.g. 0.14.
        ...
        """
        if max(s1_entropy) < self.single_reference_threshold:
            return True
        return False
