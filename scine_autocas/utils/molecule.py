...
class Molecule:
    ...
    def __init__(self,
                 ...
                 charge: int = Defaults.Molecule.charge,
                 spin_multiplicity: int = Defaults.Molecule.spin_multiplicity,
                 ecp_electrons: int = Defaults.Molecule.ecp_electrons,
                 double_d_shell: bool = Defaults.Molecule.double_d_shell,
                 unit: str = Defaults.Molecule.unit,
                 ):
        ...
        Notes
        -----
        At least one of "xyz_file", "atom_list" or "settings_dict" (with similar information) has to be provided.
        ...
        self.atoms: List[str]
        """contains the chemical element string for each atom in a molecule"""
        self.core_orbitals: int
        """the number of core orbitals"""
        self.valence_orbitals: int
        """the number of valence orbitals (occupied and virtual)"""
        self.electrons: int
        """the number of electrons"""
        ...
        self.molden_file: str = ""
        """the molden file"""

        if atom_list and xyz_file:
            print("in molecule: atoms and xyz file provided, using atoms to build molecule")

        if atom_list:
            self.atoms = atom_list
        elif xyz_file:
            self.atoms = self.get_atoms_from_xyz(xyz_file)
            self.xyz_file = xyz_file
        else:
            raise ValueError("No xyz-file or List of atom names provided")
    ...
    def __repr__(self):
        """Print molecule"""
        print_repr = "Molecule:\n  "
        print_repr += f"atoms: {self.atoms}\n  "
        print_repr += f"electrons: {self.electrons}\n  "
        print_repr += f"charge: {self.charge}\n  "
        print_repr += f"spin_multiplicity: {self.spin_multiplicity}"
        return print_repr
    ...
    def __setup_orbital_and_electrons(self, atom_list: List[str], double_d_shell: bool):
        ...
        self.electrons -= self.ecp_electrons
        self.core_orbitals -= int(self.ecp_electrons/2)
    ...
    def get_atoms_from_xyz(self, xyz_file: str) -> List[str]:
...
def from_pyscf_mol(pyscf_mol: Any) -> Molecule:
    """Build molecule object from pyscf molecule.
    ...
    -------
    mol : Molecule
        a autocas molecule object
    """

    atom_list: List[str] = []
    # pylint: disable=protected-access
    for atom in pyscf_mol._atom:
    ...
    mol = Molecule(
        ...
        spin_multiplicity=pyscf_mol.spin+1)
    return mol


def molecule_setup_from_dict(settings: Dict[str, Any]) -> Molecule:
    """Create molecule object based on settings dict.
    ...
    -------
    mol : Molecule
        a molecule object with applied settings from the dict.
    """
    mol_settings = settings["Molecule"]
    xyz_file = mol_settings["xyz_file"]
    mol = Molecule(xyz_file)
    for key in mol_settings:
        if hasattr(mol, key):
            if key in ("spin_multiplicity", "charge", "ecp_electrons"):
                mol_settings[key] = int(mol_settings[key])
            setattr(mol, key, mol_settings[key])
    mol.update()
    return mol
...
if __name__ == "__main__":
    atoms = ["N", "N"]
    molecule = Molecule(atom_list=atoms)
    print(molecule)
