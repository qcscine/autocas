...
# fromscine_autocas.io.logger import logging
...
class ThresholdPlot:
    ...
    __slots__ = ["plateau_elements", "color", "scale_by_system"]

    def __init__(self) -> None:
        """Construct a treshholdplot object."""
        self.plateau_elements: int = Defaults.AutoCAS.plateau_values
        """defines the number of elements necessary to count as a plateau"""
        self.color = "#203f78"
        self.scale_by_system = True
    ...
    def plot(self, s1_entropy: np.ndarray) -> Any:
        ...
        """
        # logging.debug("Start plotting threshold diagram")
        max_s1 = np.sqrt(2)
        if self.scale_by_system:
        ...
        # plt.figure()
        plt.plot(x_values, y_values, marker="o", markersize=3, ls="", color=self.color)
        # plt.plot(plateau_index, plateau_vector, ls="", marker="x")
        plt.ylabel("# selected Orbitals", fontsize=16)
        plt.xlabel("Threshold of largest element", fontsize=16)
        if number_of_orbitals >= 100:
            n = int(number_of_orbitals/100)*10
            plt.yticks(np.arange(0, number_of_orbitals + n, n), fontsize=16)
        elif number_of_orbitals >= 60:
            plt.yticks(np.arange(0, number_of_orbitals + 10, 10), fontsize=16)
        elif number_of_orbitals >= 10:
            plt.yticks(np.arange(0, number_of_orbitals + 5, 5), fontsize=16)
        else:
            plt.yticks(np.arange(0, number_of_orbitals + 1, 1), fontsize=16)
        # plt.yticks(np.arange(0, number_of_orbitals + 1, 20), fontsize=16)
...
if __name__ == "__main__":
    s1 = np.random.rand(305)
    plot = ThresholdPlot()
    plti = plot.plot(s1)
    plti.savefig("test.pdf")
